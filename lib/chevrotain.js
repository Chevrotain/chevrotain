...
/***/ function(module, exports, __webpack_require__) {
	...
	var follow_1 = __webpack_require__(17);
	var tokens_public_1 = __webpack_require__(10);
	...
	var Parser = (function () {
	    ...
	    Parser.prototype.optionInternal = function (predicateOrAction, action, occurrence) {
	        if (action === undefined) {
	            ...
	            predicateOrAction = this.getLookaheadFuncForOption(occurrence, predicateOrAction);
	        }
	        if (predicateOrAction.call(this)) {
	            action.call(this);
	            return true;
	        }
	        return false;
	    ...
	    Parser.prototype.atLeastOneInternal = function (prodFunc, prodName, prodOccurrence, lookAheadFunc, action, userDefinedErrMsg) {
	        if (!utils_1.isFunction(action)) {
	            ...
	            lookAheadFunc = this.getLookaheadFuncForAtLeastOne(prodOccurrence, lookAheadFunc);
	        }
	    ...
	    Parser.prototype.manyInternal = function (prodFunc, prodName, prodOccurrence, predicate, action) {
	        if (action === undefined) {
	            ...
	            predicate = this.getLookaheadFuncForMany(prodOccurrence, predicate);
	        }
	        while (predicate.call(this)) {
	            action.call(this);
	        }
	        if (this.recoveryEnabled) {
	            this.attemptInRepetitionRecovery(prodFunc, [predicate, action], predicate, prodName, prodOccurrence, interpreter_1.NextTerminalAfterManyWalker, this.manyLookaheadKeys);
	        }
	    };
	    ...
	    Parser.prototype.orInternal = function (alts, errMsgTypes, occurrence) {
	        ...
	        if (altToTake !== -1) {
	            ...
	            var grammarAction = chosenAlternative.ALT ?
	    ...
	    Parser.prototype.getLookaheadFuncForOr = function (occurrence, alts) {
	        var key = this.getKeyForAutomaticLookahead("OR", this.orLookaheadKeys, occurrence);
	        ...
	        if (laFunc === undefined) {
	            ...
	            var predicates = utils_1.map(alts, function (currAlt) { return currAlt.WHEN; });
	            laFunc = lookahead_1.buildLookaheadFuncForOr(occurrence, ruleGrammar, this.maxLookahead, predicates);
	    ...
	    Parser.prototype.getLookaheadFuncForOption = function (occurrence, predicate) {
	        var key = this.getKeyForAutomaticLookahead("OPTION", this.optionLookaheadKeys, occurrence);
	        return this.getLookaheadFuncFor(key, occurrence, lookahead_1.buildLookaheadForOption, this.maxLookahead, predicate);
	    };
	    Parser.prototype.getLookaheadFuncForMany = function (occurrence, predicate) {
	        var key = this.getKeyForAutomaticLookahead("MANY", this.manyLookaheadKeys, occurrence);
	        return this.getLookaheadFuncFor(key, occurrence, lookahead_1.buildLookaheadForMany, this.maxLookahead, predicate);
	    };
	    Parser.prototype.getLookaheadFuncForManySep = function (occurrence, predicate) {
	        var key = this.getKeyForAutomaticLookahead("MANY_SEP", this.manySepLookaheadKeys, occurrence);
	        return this.getLookaheadFuncFor(key, occurrence, lookahead_1.buildLookaheadForManySep, this.maxLookahead, predicate);
	    };
	    Parser.prototype.getLookaheadFuncForAtLeastOne = function (occurrence, predicate) {
	        var key = this.getKeyForAutomaticLookahead("AT_LEAST_ONE", this.atLeastOneLookaheadKeys, occurrence);
	        return this.getLookaheadFuncFor(key, occurrence, lookahead_1.buildLookaheadForAtLeastOne, this.maxLookahead, predicate);
	    };
	    Parser.prototype.getLookaheadFuncForAtLeastOneSep = function (occurrence, predicate) {
	        var key = this.getKeyForAutomaticLookahead("AT_LEAST_ONE_SEP", this.atLeastOneSepLookaheadKeys, occurrence);
	        return this.getLookaheadFuncFor(key, occurrence, lookahead_1.buildLookaheadForAtLeastOneSep, this.maxLookahead, predicate);
	    };
	    Parser.prototype.getLookaheadFuncFor = function (key, occurrence, laFuncBuilder, maxLookahead, predicate) {
	        var laFunc = this.classLAFuncs.get(key);
	        if (laFunc === undefined) {
	            var ruleName = utils_1.last(this.RULE_STACK);
	            var ruleGrammar = this.getGAstProductions().get(ruleName);
	...
	var Lexer = (function () {
	    ...
	    function Lexer(lexerDefinition, deferDefinitionErrorsHandling) {
	        ...
	        if (utils_1.isArray(lexerDefinition)) {
	            ...
	            actualDefinition.modes[DEFAULT_MODE] = lexerDefinition;
	            actualDefinition[DEFAULT_MODE] = DEFAULT_MODE;
	        }
	        else {
	            // some run time checks to help the end users.
	            if (!utils_1.has(lexerDefinition, DEFAULT_MODE)) {
	                this.lexerDefinitionErrors.push({
	                    message: "A MultiMode Lexer cannot be initialized without a <" + DEFAULT_MODE + "> property in its definition\n",
	                    type: LexerDefinitionErrorType.MULTI_MODE_LEXER_WITHOUT_DEFAULT_MODE
	                });
	            }
	            if (!utils_1.has(lexerDefinition, MODES)) {
	                this.lexerDefinitionErrors.push({
	                    message: "A MultiMode Lexer cannot be initialized without a <" + MODES + "> property in its definition\n",
	                    type: LexerDefinitionErrorType.MULTI_MODE_LEXER_WITHOUT_MODES_PROPERTY
	                });
	            }
	            actualDefinition = lexerDefinition;
	        }
	        // multiMode
	        ...
	        // @formatter:off
	        var allLexerModes = utils_1.has(actualDefinition, MODES) ?
	            actualDefinition.modes :
	            {};
	        // @formatter:on
	        var allModeNames = utils_1.keys(allLexerModes);
	        utils_1.forEach(allLexerModes, function (currModDef, currModName) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	function buildLookaheadFuncForOr(occurrence, ruleGrammar, k, predicates) {
	 ...
	 */
	function buildAlternativesLookAheadFunc(alts, predicates) {
	    // TODO: performance optimizations for the (common) edge case of K == 1
	    if (predicates === void 0) { predicates = []; }
	    var numOfAlts = alts.length;
	    // This version takes into account the predicates as well.
	    if (utils_1.some(predicates, function (currPred) { return utils_1.isFunction(currPred); })) {
	        /**
