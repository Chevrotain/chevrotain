    ...
    (function (lexer) {
        ...
        var SimpleLexer = (function () {
            ...
            SimpleLexer.prototype.tokenize = function (text) {
                ...
                function addLineColumnInfoTo(token) {
                    var lc = offSetToLC[offset];
                ...
                while (text.length > 0) {
                    ...
                    if (oneMatchResult !== NOTHING_CONSUMED()) {
                        ...
                        if (res !== NOTHING_CONSUMED() || text.length === 0) {
                            var lc = offSetToLC[errorStart];
                            var errorLine = lc.line;
                            var errorColumn = lc.column;
                            var errorMessage = ("unexpected character: ->" + orgInput.charAt(errorStart) + "<- at offset: " + errorStart + ",") + (" skipped " + (offset - errorStart) + " characters.");
        ...
        function NOTHING_CONSUMED() {
            return nothing_consumed;
        }
        lexer.NOTHING_CONSUMED = NOTHING_CONSUMED;
        function tokenizeOne(input, offset, patterns, patternsToConstructor) {
            var matches = _.map(patterns, function (pattern) {
                return pattern.exec(input);
            });
            var maxLength = 0;
            var biggestMatchIdx = _.reduce(matches, function (maxIdx, possibleMatch, currIdx) {
        ...
        lexer.tokenizeOne = tokenizeOne;
        function analyzeTokenClasses(tokenClasses) {
            var onlyRelevant = _.reject(tokenClasses, function (currClass) {
                return currClass[PATTERN] === lexer.NA;
            });
            var matchedClasses = _.filter(onlyRelevant, function (currClass) {
                return _.has(currClass, PATTERN) && !_.has(currClass, IGNORE);
            });
            var matchPatterns = _.map(matchedClasses, function (currClass) {
                return addStartOfInput(currClass[PATTERN]);
            });
            var ignoredClasses = _.filter(onlyRelevant, function (currClass) {
                return _.has(currClass, PATTERN) && _.has(currClass, IGNORE);
            });
            var ignorePatterns = _.map(ignoredClasses, function (currClass) {
                return addStartOfInput(currClass[PATTERN]);
            });
            var matchedPatternsToClass = _.zipObject(matchPatterns, matchedClasses);
            var ignoredPatternsToClass = _.zipObject(ignorePatterns, ignoredClasses);
            var patternToClass = {};
    ...
    (function (interpreter) {
        ...
        var NextAfterTokenWalker = (function (_super) {
            ...
            NextAfterTokenWalker.prototype.walkTerminal = function (terminal, currRest, prevRest) {
                if (this.isAtEndOfPath && t.getTokName(terminal.terminalType) === this.nextTerminalName && terminal.occurrenceInParent === this.nextTerminalOccurrence && !(this.found)) {
                    var fullRest = currRest.concat(prevRest);
                    var restProd = new g.FLAT(fullRest);
            ...
            if (!ignoreAmbiguities) {
                ...
                if (!_.isEmpty(altsAmbiguityErrors)) {
                    var errorMessages = _.map(altsAmbiguityErrors, function (currAmbiguity) {
                        return ("Ambiguous alternatives " + currAmbiguity.alts.join(" ,") + " in OR" + orOccurrence + " inside " + ruleGrammar.name + " ") + ("Rule, " + chevrotain.lang.functionName(currAmbiguity.token) + " may appears as the first Terminal in all these alternatives.\n");
                    });
