...
describe("The GAst Builder namespace", () => {
    ...
    let literalArrayText = "let lSquare = this.CONSUME1(LSquareTok)\r\n" +
    ...
    it("can find the direct 'childs' of another Production from an IProd representation", () => {
        let allProdRanges:IProdRange[] = [
            ...
            {range: new Range(30, 100), text: "1.2.1.1", type: ProdType.TERMINAL},
            {range: new Range(101, 170), text: "1.2.1.2", type: ProdType.TERMINAL},
            {range: new Range(181, 190), text: "1.2.2", type: ProdType.TERMINAL},
            {range: new Range(201, 209), text: "1.3", type: ProdType.TERMINAL}
        ]

        let topRange = getDirectlyContainedRanges(new Range(0, 210), allProdRanges)
        ...
        let orRange = getDirectlyContainedRanges(new Range(11, 200), allProdRanges)
        expect(orRange.length).to.equal(2)
        expect(orRange[0].text).to.equal("1.2.1")
        expect(orRange[1].text).to.equal("1.2.2")

        let manyRange = getDirectlyContainedRanges(new Range(20, 180), allProdRanges)
    ...
    it("can build The Gast representation of an elementDefinition Grammar Rule", () => {
        ...
        expect((<gast.Terminal>option1Def[0]).terminalType).to.equal(RequiredTok)
