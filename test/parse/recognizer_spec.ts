...
function defineRecognizerSpecs(contextName, extendToken, createToken, tokenMatcher) {
    ...
    context("Recognizer  " + contextName, () => {
        ...
        let PlusTok = extendToken("PlusTok")
        PlusTok.LABEL = "+"
        let MinusTok = extendToken("MinusTok")
        let IntTok = extendToken("IntTok")
        let DotTok = extendToken("DotTok")
        let IdentTok = extendToken("IdentTok")
        ...
        describe("The Error Recovery functionality of the Chevrotain Parser", () => {
            ...
            it("can CONSUME tokens with an index specifying the occurrence for the specific token in the current rule", () => {
                ...
                parser.reset()
                let testInput = [createToken(IntTok, "1"), createToken(PlusTok),
                    createToken(IntTok, "2"), createToken(PlusTok), createToken(IntTok, "3")]
            ...
            it("can perform in-repetition recovery for MANY grammar rule", () => {
                // a.b+.c
                let input = [createToken(IdentTok, "a"), createToken(DotTok), createToken(IdentTok, "b"),
                    createToken(PlusTok), createToken(DotTok), createToken(IdentTok, "c")]
            ...
            it("can perform in-repetition recovery for MANY_SEP grammar rule #2", () => {
                // a.b..c...d
                let input = [createToken(IdentTok, "a"), createToken(DotTok), createToken(DotTok),
                    createToken(DotTok), createToken(IdentTok, "b")]
        ...
        describe("The BaseRecognizer", () => {
            ...
            it("invoking an OPTION will return the inner grammar action's value or undefined", () => {
                ...
                class OptionsReturnValueParser extends Parser {
                    ...
                    constructor(input:Token[] = [createToken(IntTok, "666")]) {
                        super(input, ALL_TOKENS);
                        (Parser as any).performSelfAnalysis(this)
                    }
            ...
            it("will return false if a RecognitionException is thrown during backtracking and rethrow any other kind of Exception", () => {
                ...
                let throwsRecogError = () => { throw new exceptions.NotAllInputParsedException("sad sad panda", createToken(PlusTok)) }
        ...
        describe("The BaseRecognizer", () => {
            ...
            it("can be initialized with a Dictionary of Tokens", () => {
                let initTokenDictionary = {PlusTok: PlusTok, MinusTok: MinusTok, IntToken: IntTok}
                let parser:any = new Parser([], {
                    PlusTok:  PlusTok,
                    MinusTok: MinusTok,
                    IntToken: IntTok
            ...
            it("Will use Token LABELS for mismatch error messages when available", () => {
                ...
                let parser = new LabelTokParser([createToken(MinusTok)])
                parser.rule()
            ...
            it("Will not use Token LABELS for mismatch error messages when unavailable", () => {
                ...
                let parser = new NoLabelTokParser([createToken(PlusTok)])
                parser.rule()
                expect(parser.errors[0]).to.be.an.instanceof(MismatchedTokenException)
            ...
            it("Will include the ruleStack in a recognition Exception", () => {
                ...
                let parser = new NestedRulesParser([createToken(MinusTok), createToken(MinusTok)])
                parser.rule()
                expect(parser.errors[0]).to.be.an.instanceof(MismatchedTokenException)
            ...
            it("Will build an error message for AT_LEAST_ONE automatically", () => {
                ...
                let parser = new ImplicitAtLeastOneErrParser([createToken(IntTok, "666"), createToken(MinusTok), createToken(MinusTok)])
                parser.rule()
            ...
            it("Will build an error message for AT_LEAST_ONE_SEP automatically", () => {
                ...
                let parser = new ImplicitAtLeastOneSepErrParser([createToken(IntTok, "666"), createToken(MinusTok), createToken(MinusTok)])
                parser.rule()
                expect(parser.errors[0]).to.be.an.instanceof(EarlyExitException)
                expect(parser.errors[0].message).to.contain("expecting at least one iteration")
                expect(parser.errors[0].message).to.contain("MinusTok")
