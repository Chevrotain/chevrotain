...
export abstract class RestWalker {
    ...
    walk(prod:gast.AbstractProduction, prevRest:any[] = []):void {
        forEach(prod.definition, (subProd:gast.IProduction, index) => {
            ...
            else if (subProd instanceof gast.RepetitionMandatoryWithSeparator) {
    ...
    walkFlat(flatProd:gast.Flat, currRest:gast.IProduction[], prevRest:gast.IProduction[]):void {
    ...
    walkAtLeastOne(atLeastOneProd:gast.RepetitionMandatory, currRest:gast.IProduction[], prevRest:gast.IProduction[]):void {
        ...
        let fullAtLeastOneRest:gast.IProduction[] = [new gast.Option(atLeastOneProd.definition)].concat(<any>currRest, <any>prevRest)
    ...
    walkMany(manyProd:gast.Repetition, currRest:gast.IProduction[], prevRest:gast.IProduction[]):void {
        // ABC(DE)*F => after the (DE)* the rest is (DE)?F
        let fullManyRest:gast.IProduction[] = [new gast.Option(manyProd.definition)].concat(<any>currRest, <any>prevRest)
...
function restForRepetitionWithSeparator(repSepProd, currRest, prevRest) {
    let repSepRest = [new gast.Option([<any>new gast.Terminal(repSepProd.separator)].concat(repSepProd.definition))]
    let fullRepSepRest:gast.IProduction[] = repSepRest.concat(<any>currRest, <any>prevRest)
