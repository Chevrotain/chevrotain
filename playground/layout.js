...
function dragMiddleHorz(event) {
  ...
  if (percentageLeft) {
    ...
    percentageLeft =
      percentageLeft > 99.26 - SNAP_TO_DISTANCE ? 99.26 : percentageLeft;
...
var resizeVerticalLeft = _.throttle(
  _.partialRight(
    dragVertical,
    "impel",
    "input",
...
var resizeVerticalRight = _.throttle(
  _.partialRight(
    dragVertical,
    "rightTop",
    "output",
...
function dragVertical(event, topID, buttomID, topCmID, buttomCmID) {
  ...
  if (percentageTop) {
    ...
    percentageTop =
      percentageTop > PAGE_VH - SNAP_TO_DISTANCE ? PAGE_VH : percentageTop;
  ...
  function resizeTick() {
    setTimeout(function () {
      if (currDiagramsDivPercentage > 0) {
        var outputVh =
          "" + (PAGE_VH - currDiagramsDivPercentage + tickSize) + "vh";
        ...
        $("#rightTop").height("0vh");
        $("#outputEditorDiv").height(
          PAGE_VH + H_SEPARATOR_VH - HEADER_BOX_VH + "vh"
        );
        refreshCodeMirrorInstances();
    ...
    setTimeout(function () {
      if (currOutputDivPercentage > lastOutputDivPercentage) {
        ...
        if (currOutputDivPercentage - tickSize < lastOutputDivPercentage) {
          ...
          var diagramsVh =
            "" + (PAGE_VH - currOutputDivPercentage + tickSize) + "vh";
