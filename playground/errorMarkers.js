...
function markLexerDefinitionErrors(lexer) {
  ...
  _.forEach(lexer.lexerDefinitionErrors, function (currLexError) {
    errPositions = getLexerErrorStartStopPos(
      currLexError,
...
function markParserDefinitionErrors(parser) {
  ...
  _.forEach(parser.definitionErrors, function (currParserDefError) {
    errPositions = getParserErrorStartStopPos(
      currParserDefError,
      javaScriptEditor.getValue(),
      parser,
      javaScriptEditor
...
function getLexerErrorStartStopPos(lexErr, parserImplText, positionHelper) {
  switch (lexErr.type) {
    case chevrotain.LexerDefinitionErrorType.DUPLICATE_PATTERNS_FOUND:
      return locateRegExpLiteral(
        _.first(lexErr.tokenClasses).PATTERN,
        parserImplText,
        positionHelper,
        lexErr.tokenClasses.length
      );
      break;
    case chevrotain.LexerDefinitionErrorType.INVALID_PATTERN:
    case chevrotain.LexerDefinitionErrorType.MISSING_PATTERN:
      return [
        locateExtendTokenPos(
          _.first(lexErr.tokenClasses),
          parserImplText,
          positionHelper
        ),
      ];
 ...
 *             end:{line:number, column:number}[]}
 */
function getParserErrorStartStopPos(
  parseErr,
  parserImplText,
  parser,
  positionHelper
  ...
  switch (parseErr.type) {
    ...
    case chevrotain.ParserDefinitionErrorType.UNRESOLVED_SUBRULE_REF:
      ...
      return locateSubruleRef(
        parserImplText,
        parseErr.unresolvedRefName,
        positionHelper,
        ruleText
      );
...
function locateTokenDefinition(paramName, text, positionHelper) {
  ...
  var patternRuleNameGroup = 2;
  var soughtPattern =
    "(createToken\\s*\\(\\s*\\{\\s*name\\s*:\\s*)(['\"]" + paramName + "['\"])";
  var seekerRegExp = RegExp(soughtPattern, "g");
...
var locateAtLeastOneSepSeparator = _.partial(
  locateGrammarUsage,
  "AT_LEAST_ONE_SEP"
);
...
function locateGrammarUsage(
  ...
  var soughtPattern =
    "(\\." + dslName + occurrenceIdx + "\\s*\\(.*)(" + paramName + ")\\W";
  var seekerRegExp = RegExp(soughtPattern, "g");
  ...
  while ((execResult = seekerRegExp.exec(text))) {
    var startOffset =
      textStartOffset +
      execResult.index +
      execResult[patternPrefixGroup].length;
...
function locateProduction(
  ...
  var dslNameWithOccurrence =
    dslName + (occurrence === 0 ? "(?:|" + occurrence + ")" : occurrence);
