...
function loadExample(exampleName, firstTime) {
    ...
    let sampleText =
        "(" + sample.implementation.toString().replace(/    /g, "  ") + "())";
...
function updateSamplesDropDown() {
    ...
    _.forOwn(
        samples[examplesDropdown.val()].sampleInputs,
                ...
                $.OR([
                    {
                        ALT: () => {
                            const stringLiteral =
                                $.CONSUME(StringLiteral).image;
                            // chop of the quotation marks
                            return stringLiteral.substr(
                                1,
