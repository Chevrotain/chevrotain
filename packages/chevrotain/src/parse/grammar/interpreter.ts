...
export abstract class AbstractNextPossibleTokensWalker extends RestWalker {
  ...
  startWalking(): TokenType[] {
    ...
    this.ruleStack = this.path.ruleStack.slice().reverse() // intelij bug requires assertion
    this.occurrenceStack = this.path.occurrenceStack.slice().reverse() // intelij bug requires assertion
...
export class NextTerminalAfterManyWalker extends AbstractNextTerminalAfterProductionWalker {
  walkMany(
    ...
    if (manyProd.idx === this.occurrence) {
      const firstAfterMany = currRest[0] ?? prevRest[0]
...
export class NextTerminalAfterManySepWalker extends AbstractNextTerminalAfterProductionWalker {
  walkManySep(
    ...
    if (manySepProd.idx === this.occurrence) {
      const firstAfterManySep = currRest[0] ?? prevRest[0]
...
export class NextTerminalAfterAtLeastOneWalker extends AbstractNextTerminalAfterProductionWalker {
  walkAtLeastOne(
    ...
    if (atLeastOneProd.idx === this.occurrence) {
      const firstAfterAtLeastOne = currRest[0] ?? prevRest[0]
...
export class NextTerminalAfterAtLeastOneSepWalker extends AbstractNextTerminalAfterProductionWalker {
  walkAtLeastOneSep(
    ...
    if (atleastOneSepProd.idx === this.occurrence) {
      const firstAfterfirstAfterAtLeastOneSep = currRest[0] ?? prevRest[0]
...
export function nextPossibleTokensAfter(
  ...
  while (possiblePaths.length > 0) {
    ...
    if (currPath === EXIT_ALTERNATIVE) {
      if (
        foundCompletePath &&
        possiblePaths[possiblePaths.length - 1].idx <= minimalAlternativesIndex
      ) {
