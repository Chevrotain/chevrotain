    ...
    context("nested rules", () => {
      context("Can output cst when using OPTION", () => {
        ...
        it("path taken", () => {
          ...
          let $nestedOptionCst = cst.children.$nestedOption[0] as CstNode
          expect(tokenStructuredMatcher($nestedOptionCst.children.A[0], A)).to
            .be.true
      ...
      it("Can output a CST when using OR with nested named Alternatives", () => {
        ...
        expect(tokenStructuredMatcher(firstAltCst.children.A[0], A)).to.be.true
        expect(cst.children.bamba).to.be.undefined
      ...
      it("Can output a CST when using OR - single Alt", () => {
        ...
        expect((altCst.children.bamba[0] as CstNode).children).to.have.keys("C")
      })
      ...
      it("Can output a CST using Repetitions", () => {
        ...
        expect(tokenStructuredMatcher(bamba[0].children.C[0], C)).to.be.true
        expect(tokenStructuredMatcher(bamba[1].children.C[0], C)).to.be.true
        expect(tokenStructuredMatcher(bamba[2].children.C[0], C)).to.be.true
      })
      ...
      it("Can output a CST using Repetitions with separator - mandatory", () => {
        ...
        expect(tokenStructuredMatcher(nestedCst.children.A[0], A)).to.be.true
        expect(tokenStructuredMatcher(nestedCst.children.A[1], A)).to.be.true
        expect(nestedCst.children.C).to.have.length(1)
        expect(tokenStructuredMatcher(nestedCst.children.C[0], C)).to.be.true
      })
