/**
 * CSS grammar factory for benchmarking.
 * Mirrors the implementation in examples/grammars/css/css.js
 * but accepts a chevrotain module to allow version-swapping.
 *
 * Based on the W3C CSS 2.1 grammar:
 * https://www.w3.org/TR/CSS21/grammar.html
 */
import XRegExp from "xregexp";
import type { GrammarFactory } from "../types.ts";

export const createCssBenchmark: GrammarFactory = (chevrotain, options) => {
  const { createToken: orgCreateToken, CstParser, Lexer } = chevrotain;

  // ---------- XRegExp fragment DSL ----------
  ...
  // ---------- Token auto-collection wrapper ----------
  const cssTokens: any[] = [];
  const createToken = function (...args: any[]) {
  ...
  const parser = new CssParser() as any;
